<?php
    ...
    $select= "SELECT id,listing_title,featured FROM `listings` where is_active=1";
    ...
    if(mysqli_num_rows($result_count)>0){
        ...
        while($val =  mysqli_fetch_assoc($result_count)){
            if($val['featured']==1){
                $arr[$i]= array("name"=>$val['listing_title'],"abbreviation"=>"al_".$val['id'],"checked_val" => "yes","id"=>$val['id']);
            }else{
                $arr[$i]= array("name"=>$val['listing_title'],"abbreviation"=>"al_".$val['id'],"checked_val" => "no","id"=>$val['id']);
            } 
            $i++;
        }
    }
/*
    ...
    if(mysqli_num_rows($result_count)>0){
        ...
        while($val =  mysqli_fetch_assoc($result_count)){
            if($val['popular']==1){
                $arr_makes[$i]= array("name"=>$val['title'],"abbreviation"=>"al_".$val['id'],"checked_val" => "yes","id"=>$val['id']);
            }else{
                $arr_makes[$i]= array("name"=>$val['title'],"abbreviation"=>"al_".$val['id'],"checked_val" => "no","id"=>$val['id']);
            } 
            $i++;
        }
    } */
		...
		<div class="container-fluid">
			<div class="row">
              ...
              <div class="breadcrumb_content mb50">
                <h2 class="breadcrumb_title">Manage Featured Listings</h2>
                      ...
                      <div class="shortcode_widget_form">
                        <div class="ui_kit_input mb25">
                          <form method="post" action="controlling.php" enctype="multipart/form-data">
                            <input type="hidden" name="cmd" value="featured_listings" />
                            <div class="form-group p-1">
                          
                                <div class="dropdown-container">
                                    <div class="dropdown-button noselect">
                                        <div class="dropdown-label">Set Popular Listings (<span class="quantity">Any</span>)</div>
...
var stateTemplate = _.template(
    '<li>' +
    	'<input name="featured_listings[]" id="<%= abbreviation %>" value="<%= id %>" type="checkbox"  <%= chek_kk %>>' +
